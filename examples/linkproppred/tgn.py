...
def train():
    ...
    for batch in tqdm(train_loader, desc="Training", total=len(train_loader)):
        ...
        assoc_popular[n_id] = torch.arange(n_id.size(0), device=device)
...
@torch.no_grad()
def test(loader, neg_sampler, split_mode, popular_negatives, src_dst_t_to_index):
    r"""
    ...
    for batch_ix, pos_batch in tqdm(enumerate(loader), desc="Testing", total=len(loader)):
        ...
        if batch_ix > 0 and batch_ix % 1000 == 0:
            ...
            tqdm.write(f"Oversaturated Naive Sampling: {float(torch.tensor(oversaturated_naive_sampling).mean())*100:.4f}%")
            tqdm.write(
            ...
            if split_mode == "val":
                tqdm.write(f"Oversaturated K50 N5000: {float(torch.tensor(oversaturated_K50_N5000).mean())*100:.4f}%")
                tqdm.write(f"Oversaturated K100 N5000: {float(torch.tensor(oversaturated_K100_N5000).mean())*100:.4f}%")
                tqdm.write(f"Oversaturated K1000 N5000: {float(torch.tensor(oversaturated_K1000_N5000).mean())*100:.4f}%")
                tqdm.write(f"Oversaturated K50 N20000: {float(torch.tensor(oversaturated_K50_N20000).mean())*100:.4f}%")
                tqdm.write(f"Oversaturated K100 N20000: {float(torch.tensor(oversaturated_K100_N20000).mean())*100:.4f}%")
                tqdm.write(f"Oversaturated K1000 N20000: {float(torch.tensor(oversaturated_K1000_N20000).mean())*100:.4f}%")
                tqdm.write(f"Oversaturated K50 N100000: {float(torch.tensor(oversaturated_K50_N100000).mean())*100:.4f}%")
                tqdm.write(f"Oversaturated K100 N100000: {float(torch.tensor(oversaturated_K100_N100000).mean())*100:.4f}%")
                tqdm.write(f"Oversaturated K1000 N100000: {float(torch.tensor(oversaturated_K1000_N100000).mean())*100:.4f}%")

    perf_metrics = float(torch.tensor(perf_list).mean())

    print(f"Naive MRR: {perf_metrics}")
    ...
    print(f"Oversaturated Popular Top 20 Sampling: {float(torch.tensor(oversaturated_popular_top20_sampling).mean())*100:.4f}%")
    print(
        f"Oversaturated Popular Top 100 Sampling: {float(torch.tensor(oversaturated_popular_top100_sampling).mean())*100:.4f}%"
    )
    print(
        f"Oversaturated Popular Top 500 Sampling: {float(torch.tensor(oversaturated_popular_top500_sampling).mean())*100:.4f}%"
    )
    if split_mode == "val":
        print(f"Oversaturated K50 N5000: {float(torch.tensor(oversaturated_K50_N5000).mean())*100:.4f}%")
        print(f"Oversaturated K100 N5000: {float(torch.tensor(oversaturated_K100_N5000).mean())*100:.4f}%")
        print(f"Oversaturated K1000 N5000: {float(torch.tensor(oversaturated_K1000_N5000).mean())*100:.4f}%")
        print(f"Oversaturated K50 N20000: {float(torch.tensor(oversaturated_K50_N20000).mean())*100:.4f}%")
        print(f"Oversaturated K100 N20000: {float(torch.tensor(oversaturated_K100_N20000).mean())*100:.4f}%")
        print(f"Oversaturated K1000 N20000: {float(torch.tensor(oversaturated_K1000_N20000).mean())*100:.4f}%")
        print(f"Oversaturated K50 N100000: {float(torch.tensor(oversaturated_K50_N100000).mean())*100:.4f}%")
        print(f"Oversaturated K100 N100000: {float(torch.tensor(oversaturated_K100_N100000).mean())*100:.4f}%")
        print(f"Oversaturated K1000 N100000: {float(torch.tensor(oversaturated_K1000_N100000).mean())*100:.4f}%")
...
assoc_popular = torch.empty(data.num_nodes, dtype=torch.long, device=device)
