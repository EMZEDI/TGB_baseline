...
def test_the_same_scores_20_negatives():
    ...
    expected_mrr = 0.1735
    assert np.isclose(evaluator.eval(input_dict, average_rank=False)["mrr"], expected_mrr, atol=1e-4)

    expected_mrr = 1 / 11
    assert np.isclose(evaluator.eval(input_dict, average_rank=True)["mrr"], expected_mrr, atol=1e-4)
